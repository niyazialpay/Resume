...
class firewall{
    ...
    public function handle(Request $request, Closure $next){
        ...
        define('PHP_FIREWALL_REQUEST_URI', $request->getRequestUri());
        ...
        define('PHP_FIREWALL_ADMIN_MAIL', 'muhammed@niyazialay.com' );
        ...
        /** IPS PROTECTED */
        if ( count( $IP_DISALLOW ) > 0 ) {
            if ( in_array( DetectIPAddress(), $IP_DISALLOW ) ) return abort(403);
        }
        /** END IPS PROTECTED */
        ...
        if ( PHP_FIREWALL_ACTIVATION === true ) {
            ...
            if ( PHP_FIREWALL_PROTECTION_UNSET_GLOBALS === true ) $this->PHP_FIREWALL_unset_globals();



            /** bases define */
            define('PHP_FIREWALL_GET_QUERY_STRING', strtolower( $this->PHP_FIREWALL_get_query_string() ) );
            define('PHP_FIREWALL_USER_AGENT', $this->PHP_FIREWALL_get_user_agent() );
            define('PHP_FIREWALL_GET_IP', $this->PHP_FIREWALL_get_ip() );
            define('PHP_FIREWALL_GET_HOST', $this->PHP_FIREWALL_gethostbyaddr() );
            define('PHP_FIREWALL_GET_REFERER', $this->PHP_FIREWALL_get_referer() );
            define('PHP_FIREWALL_GET_REQUEST_METHOD', $this->PHP_FIREWALL_get_request_method() );
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_OVH === true ) {
                if ( stristr( PHP_FIREWALL_GET_HOST ,'ovh') ) {
                    $this->PHP_FIREWALL_LOGS( 'OVH Server list' );
                    ##abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_OVH_BY_IP === true ) {
                ...
                if ( $ip[0].'.'.$ip[1] == '87.98' or  $ip[0].'.'.$ip[1] == '91.121' or  $ip[0].'.'.$ip[1] == '94.23' or $ip[0].'.'.$ip[1] == '213.186' or  $ip[0].'.'.$ip[1] == '213.251' ) {
                    $this->PHP_FIREWALL_LOGS( 'OVH Server IP' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_KIMSUFI === true ) {
                if ( stristr( PHP_FIREWALL_GET_HOST ,'kimsufi') ) {
                    $this->PHP_FIREWALL_LOGS( 'KIMSUFI Server list' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_DEDIBOX === true ) {
                if ( stristr( PHP_FIREWALL_GET_HOST ,'dedibox') ) {
                    $this->PHP_FIREWALL_LOGS( 'DEDIBOX Server list' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_DEDIBOX_BY_IP === true ) {
                $ip = explode('.', PHP_FIREWALL_GET_IP );
                if ( $ip[0].'.'.$ip[1] == '88.191' ) {
                    $this->PHP_FIREWALL_LOGS( 'DEDIBOX Server IP' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_DIGICUBE === true ) {
                if ( stristr( PHP_FIREWALL_GET_HOST ,'digicube') ) {
                    $this->PHP_FIREWALL_LOGS( 'DIGICUBE Server list' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_SERVER_DIGICUBE_BY_IP === true ) {
                $ip = explode('.', PHP_FIREWALL_GET_IP );
                if ( $ip[0].'.'.$ip[1] == '95.130' ) {
                    $this->PHP_FIREWALL_LOGS( 'DIGICUBE Server IP' );
                    #abort(403);
                }
            }

            if ( PHP_FIREWALL_PROTECTION_RANGE_IP_SPAM === true ) {
                $ip_array = array('24', '186', '189', '190', '200', '201', '202', '209', '213', '217', '222' );
                $range_ip = explode('.', PHP_FIREWALL_GET_IP );
                if ( in_array( $range_ip[0], $ip_array ) ) {
                    $this->PHP_FIREWALL_LOGS( 'IPs Spam list' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_COOKIES === true ) {
                ...
                if ( PHP_FIREWALL_PROTECTION_POST === true ) {
                    foreach( $request->post() as $value ) {
                        $check = @str_replace($ct_rules, '*', $value);
                        if( $value != $check ) {
                            $this->PHP_FIREWALL_LOGS( 'POST protect' );
                            unset( $value );
                        }
                    }
                }
                if ( PHP_FIREWALL_PROTECTION_GET === true ) {
                    foreach( $_GET as $value ) {
                        $check = str_replace($ct_rules, '*', $value);
                        if( $value != $check ) {
                            $this->PHP_FIREWALL_LOGS( 'GET protect' );
                            unset( $value );
                        }
                    }
                }
                ...
                if ( PHP_FIREWALL_GET_REQUEST_METHOD == 'POST' ) {
                    if (isset($_SERVER['HTTP_REFERER'])) {
                        if ( ! stripos( $_SERVER['HTTP_REFERER'], $_SERVER['HTTP_HOST'], 0 ) ) {
                            $this->PHP_FIREWALL_LOGS( 'Posting another server' );
                            #abort(403);
                        }
                    }
                }
            ...
            if ( PHP_FIREWALL_PROTECTION_BOTS === true ) {
                ...
                if( strtolower(PHP_FIREWALL_USER_AGENT) != $check ) {
                    $this->PHP_FIREWALL_LOGS( 'Bots attack' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_REQUEST_METHOD === true ) {
                if(strtolower(PHP_FIREWALL_GET_REQUEST_METHOD)!='get' AND strtolower(PHP_FIREWALL_GET_REQUEST_METHOD)!='head' AND strtolower(PHP_FIREWALL_GET_REQUEST_METHOD)!='post' AND strtolower(PHP_FIREWALL_GET_REQUEST_METHOD)!='put') {
                    $this->PHP_FIREWALL_LOGS( 'Invalid request' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_DOS === true ) {
                if ( !defined('PHP_FIREWALL_USER_AGENT')  || PHP_FIREWALL_USER_AGENT == '-' ) {
                    $this->PHP_FIREWALL_LOGS( 'Dos attack' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_UNION_SQL === true ) {
                ...
                if ( !empty( $stop ) ) {
                    $this->PHP_FIREWALL_LOGS( 'Union attack' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_CLICK_ATTACK === true ) {
                ...
                if( PHP_FIREWALL_GET_QUERY_STRING != str_replace($ct_rules, '*', PHP_FIREWALL_GET_QUERY_STRING ) ) {
                    $this->PHP_FIREWALL_LOGS( 'Click attack' );
                    #abort(403);
                }
            }
            ...
            if ( PHP_FIREWALL_PROTECTION_XSS_ATTACK === true ) {
                $ct_rules = array( 'http\:\/\/', 'https\:\/\/', 'cmd=', '&cmd', 'exec', 'concat', './', '../',  'http:', 'h%20ttp:', 'ht%20tp:', 'htt%20p:', 'http%20:', 'https:', 'h%20ttps:', 'ht%20tps:', 'htt%20ps:', 'http%20s:', 'https%20:', 'ftp:', 'f%20tp:', 'ft%20p:', 'ftp%20:', 'ftps:', 'f%20tps:', 'ft%20ps:', 'ftp%20s:', 'ftps%20:', '.php?url=', 'act', '&act', 'act=', '&act=','chr(', 'chr=', 'chr%20', '%20chr', 'wget%20', '%20wget', 'wget(',
                    ...
                    'select from', 'drop%20', '.system', 'getenv', 'http_', '_php', 'php_', 'phpinfo()', '<?php', '?>', 'sql=', '%f6','%f22' );
                $check    = str_replace($ct_rules, '*', PHP_FIREWALL_GET_QUERY_STRING );
                if( PHP_FIREWALL_GET_QUERY_STRING != $check ) {
                    $this->PHP_FIREWALL_LOGS( 'XSS attack' );
                    #abort(403);
                }
            }
    ...
    private function PHP_FIREWALL_get_referer() {
        if( $this->PHP_FIREWALL_get_env('HTTP_REFERER') )
            return $this->PHP_FIREWALL_get_env('HTTP_REFERER');
        return 'no referer';
    }

    private function PHP_FIREWALL_get_ip() {
    ...
    private function PHP_FIREWALL_get_user_agent() {
        if($this->PHP_FIREWALL_get_env('HTTP_USER_AGENT'))
            return $this->PHP_FIREWALL_get_env('HTTP_USER_AGENT');
        return 'none';
    }

    private function PHP_FIREWALL_get_query_string() {
        if( $this->PHP_FIREWALL_get_env('QUERY_STRING') )
            return str_replace('%09', '%20', $this->PHP_FIREWALL_get_env('QUERY_STRING'));
        return '';
    }

    private function PHP_FIREWALL_get_request_method() {
        if($this->PHP_FIREWALL_get_env('REQUEST_METHOD'))
            return $this->PHP_FIREWALL_get_env('REQUEST_METHOD');
        return 'none';
    }

    private function PHP_FIREWALL_gethostbyaddr() {
        if ( PHP_FIREWALL_PROTECTION_SERVER_OVH === true or PHP_FIREWALL_PROTECTION_SERVER_KIMSUFI === true or PHP_FIREWALL_PROTECTION_SERVER_DEDIBOX === true or PHP_FIREWALL_PROTECTION_SERVER_DIGICUBE === true ) {
            if ( @ empty( $_SESSION['PHP_FIREWALL_gethostbyaddr'] ) ) {
                return $_SESSION['PHP_FIREWALL_gethostbyaddr'] = @gethostbyaddr( $this->PHP_FIREWALL_get_ip() );
            } else {
    ...
    private function PHP_FIREWALL_push_email( $subject, $msg ) {
        $data = array(
            'message'   =>  $msg,
        ...
        Mail::to(env('CONTACT_EMAIL'))->send(new SendMail($data));
    }

    private function PHP_FIREWALL_LOGS( $type ) {
        $f = fopen( $_SERVER["DOCUMENT_ROOT"].'/storage/logs/'.PHP_FIREWALL_LOG_FILE.'.txt', 'a');
        $msg = date('j-m-Y H:i:s')." | $type | IP: ".PHP_FIREWALL_GET_IP." ] | DNS: ".gethostbyaddr(PHP_FIREWALL_GET_IP)." | Agent: ".PHP_FIREWALL_USER_AGENT." | URL: ".env('APP_URL').PHP_FIREWALL_REQUEST_URI." | Referer: ".PHP_FIREWALL_GET_REFERER."\n\n";
        fputs($f, $msg);
        fclose($f);
        if ( PHP_FIREWALL_PUSH_MAIL === true ) {
            $this->PHP_FIREWALL_push_email( 'Alert PHP Firewall '.strip_tags( $_SERVER['SERVER_NAME'] ) , "PHP Firewall logs of ".strip_tags( $_SERVER['SERVER_NAME'] )."\n".str_replace('|', "\n", $msg ) );
        }
    }

    private function PHP_FIREWALL_unset_globals() {
